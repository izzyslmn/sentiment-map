...
# Protect against crash on min/max
if "pubyear" in df.columns and not df["pubyear"].empty and df["pubyear"].dtype in ["int64", "float64"]:
    try:
        year_values = df["pubyear"].dropna().tolist()
        if len(year_values) > 0:
            min_year = int(min(year_values))
            max_year = int(max(year_values))
            ...
            st.sidebar.warning("No usable years available.")
    except:
        st.sidebar.warning("Could not process pubyear values. Slider disabled.")
else:
