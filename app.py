...
# Check if 'pubyear' is usable before applying filter
if "pubyear" in df.columns and pd.api.types.is_numeric_dtype(df["pubyear"]):
    clean_years = df["pubyear"].dropna().astype(int)
    if not clean_years.empty:
        min_year = clean_years.min()
        max_year = clean_years.max()
        ...
        st.sidebar.warning("No valid year data to filter.")
else:
    st.sidebar.warning("⚠️ 'pubyear' column missing or non-numeric. Year filtering disabled.")
