...
def load_data():
    ...
    if "pubyear" in df.columns:
        # Clean the pubyear column more aggressively
...
# Year filtering with protection
if "pubyear" in df.columns and not df["pubyear"].empty:
    try:
        ...
        st.sidebar.warning("Could not filter by year due to invalid data.")
else:
    st.sidebar.warning("No usable year data found. Year filter disabled.")
