...
def load_data():
    ...
    # Sanitize and clean pubyear
    if "pubyear" in df.columns:
        df["pubyear"] = pd.to_numeric(df["pubyear"], errors="coerce")  # convert to numeric or NaN
        df = df.dropna(subset=["pubyear"])  # drop rows where pubyear couldn't be parsed
        df["pubyear"] = df["pubyear"].astype(int)  # convert to int
...
# Year filter with full protection
if "pubyear" in df.columns and not df["pubyear"].empty:
    if df["pubyear"].count() > 0:
        ...
        st.sidebar.warning("No usable publication year data available.")
else:
